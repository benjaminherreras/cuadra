...
function parseDate(v: string): Date {
  ...
  if (!m) throw new InvalidCSVError(`Fecha Scotiabank no reconocida: ${v}`)
  return new Date(`${m[3]}-${m[2]}-${m[1]}`)
}

export class ScotiabankParser implements IBankParser {
  ...
  canParse(fileContent: Buffer): boolean {
    ...
    return first.includes('scotiabank') || (first.includes('fecha') && first.includes('cargo') && first.includes('abono'))
  }
  async parse(fileContent: Buffer, sessionId: string): Promise<TransaccionBancaria[]> {
    ...
    catch (e) { throw new InvalidCSVError(`CSV Scotiabank inválido: ${String(e)}`) }
    const out: TransaccionBancaria[] = []
    for (const row of rows) {
      const r: Row = {}; for (const [k, v] of Object.entries(row)) r[norm(k)] = v
      if (!r['fecha']) continue
      const cargo = amt(r['cargo'] ?? '0'); const abono = amt(r['abono'] ?? '0')
      const monto = abono > 0 ? abono : -cargo
      if (monto === 0) continue
      out.push({ id: uuidv4(), sessionId, fecha: parseDate(r['fecha']!), descripcion: r['descripcion'] ?? '', monto, banco: this.banco, rawLine: Object.values(row).join(delim) })
    }
    return out
  }
}
