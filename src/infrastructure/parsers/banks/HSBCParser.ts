...
function parseDate(val: string): Date {
  ...
  throw new InvalidCSVError(`Formato de fecha HSBC no reconocido: ${val}`)
}
...
export class HSBCParser implements IBankParser {
  readonly banco = 'HSBC'
  ...
  canParse(fileContent: Buffer): boolean {
    ...
    return firstLine.includes('debito') && firstLine.includes('credito')
  }
  ...
  async parse(fileContent: Buffer, sessionId: string): Promise<TransaccionBancaria[]> {
    ...
    try {
      ...
      throw new InvalidCSVError(`No se pudo parsear el CSV HSBC: ${String(err)}`)
    }

    if (rows.length === 0) throw new InvalidCSVError('CSV HSBC vacío o sin filas de datos')
    ...
    for (const row of rows) {
      ...
      const debito = parseAmount(nr['debito'] ?? '0')
      const credito = parseAmount(nr['credito'] ?? '0')
