...
function parseDate(val: string): Date {
  ...
  throw new InvalidCSVError(`Formato de fecha Santander no reconocido: ${val}`)
}
...
export class SantanderParser implements IBankParser {
  ...
  canParse(fileContent: Buffer): boolean {
    ...
    return firstLine.includes('movimiento') && firstLine.includes('importe')
  }
  ...
  async parse(fileContent: Buffer, sessionId: string): Promise<TransaccionBancaria[]> {
    ...
    try {
      ...
      throw new InvalidCSVError(`No se pudo parsear el CSV Santander: ${String(err)}`)
    }

    if (rows.length === 0) throw new InvalidCSVError('CSV Santander vacío o sin filas de datos')
    ...
    for (const row of rows) {
      ...
      const importe = parseAmount(nr['importe'] ?? '0')
      if (importe === 0) continue

      // Santander: columna "movimiento" = "ABONO" → ingreso, "CARGO" → egreso
      const movimiento = (nr['movimiento'] ?? '').toUpperCase()
      const monto = movimiento === 'CARGO' ? -importe : importe
