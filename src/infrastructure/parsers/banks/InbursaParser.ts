...
function parseDate(v: string): Date {
  ...
  if (!m) throw new InvalidCSVError(`Fecha Inbursa no reconocida: ${v}`)
  return new Date(`${m[3]}-${m[2]}-${m[1]}`)
}

export class InbursaParser implements IBankParser {
  ...
  canParse(fileContent: Buffer): boolean {
    ...
    return first.includes('inbursa') || (first.includes('fecha') && first.includes('cargo') && first.includes('abono') && !first.includes('deposito'))
  }
  async parse(fileContent: Buffer, sessionId: string): Promise<TransaccionBancaria[]> {
    ...
    catch (e) { throw new InvalidCSVError(`CSV Inbursa inválido: ${String(e)}`) }
