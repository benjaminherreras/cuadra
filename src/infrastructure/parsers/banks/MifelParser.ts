...
function parseDate(v: string): Date {
  ...
  if (!m) throw new InvalidCSVError(`Fecha Mifel no reconocida: ${v}`)
  return new Date(`${m[3]}-${m[2]}-${m[1]}`)
}

export class MifelParser implements IBankParser {
  ...
  async parse(fileContent: Buffer, sessionId: string): Promise<TransaccionBancaria[]> {
    ...
    catch (e) { throw new InvalidCSVError(`CSV Mifel inválido: ${String(e)}`) }
