...
function parseDate(v: string): Date {
  ...
  if (!m) throw new InvalidCSVError(`Fecha BanBajío no reconocida: ${v}`)
  return new Date(`${m[3]}-${m[2]}-${m[1]}`)
}

export class BanBajioParser implements IBankParser {
  ...
  canParse(fileContent: Buffer): boolean {
    ...
    return first.includes('bajio') || first.includes('bajío')
  }
  async parse(fileContent: Buffer, sessionId: string): Promise<TransaccionBancaria[]> {
    ...
    catch (e) { throw new InvalidCSVError(`CSV BanBajío inválido: ${String(e)}`) }
